...
export async function GET() {
  const sql = `-- Create new normalized database schema for MindDumper
...
SELECT 'New normalized database schema created successfully!' as result,
       'Original trigger_words table is preserved for migration' as note;`
  ...
  return NextResponse.json({ 
    ...
    steps: [
      ...
      '4. Click Run',
      '5. The new normalized tables will be created alongside existing tables'
    ],
    note: 'This preserves your existing trigger_words table for future migration'
  })
}
